...
# /home/jason/git_repositories/HandBrake_Scripts/HB_DashCam_Converter_SD_Copy.sh
...
#if [ ! -d "$SDSOURCE" ]
#then
#	echo SD folder at "$SDSOURCE" is not valid. Checking if default folder of /media/jason/SD_VOL/DCIM is valid.
#	SDSOURCE="/media/jason/SD_VOL/DCIM"
#elif [ ! -d "$SDSOURCE" ]
#then
#	echo Default SD folder of /media/jason/SD_VOL/DCIM is not valid either.
#	echo Terminating script using exit 2.
#	exit 2
#else
#	SDSOURCE="/media/jason/SD_VOL/DCIM"
#	echo Default SD folder is good.  Using $SDSOURCE as SD source folder
#fi



#if [ ! -d "$DEST" ] ; then
#	echo Destination working folder at "$MMVHBDEST" is not valid
#	exit 2
#fi
...
#TRANSCODEDIR="$SDSOURCE"

cp -p -r -v "$SDSOURCE"/* "$DEST"

rmdir -v "$DEST"/*

/home/jason/git_repositories/HandBrake_Scripts/HB_DashCam_Converter-Production_v1.sh "$DEST"

#find "$SDSOURCE"/* -type f -exec bash -c 'echo "$1"' __ {} \;
